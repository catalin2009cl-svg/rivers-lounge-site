...
interface Props {
  initialOrders: Order[];
  pendingFreeCode?: string | null; // ← NOU: codul FREE neafișat încă
}

export function MyOrdersClient({ initialOrders, pendingFreeCode }: Props) {
  ...
  useEffect(() => {
    if (!pendingFreeCode) return;
    // Afișăm o singură dată per sesiune browser
    const seenKey = `rl-celebration-${pendingFreeCode}`;
    if (sessionStorage.getItem(seenKey)) return;
    sessionStorage.setItem(seenKey, '1');
    setCelebrationCode(pendingFreeCode);
