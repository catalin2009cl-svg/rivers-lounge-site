...
import {
  uploadRiversLandPhoto,
  deleteRiversLandPhoto,
  reorderRiversLandPhotos,
} from '@/lib/actions/gallery';
import type { RiversLandPhoto } from '@/lib/server-data';
...
interface Props {
  initialPhotos: RiversLandPhoto[];
}

export function RiversLandGalleryClient({ initialPhotos }: Props) {
  const [photos, setPhotos] = useState<RiversLandPhoto[]>(initialPhotos);
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    for (const raw of arr) {
      ...
      fd.append('file', file);
      const result = await uploadRiversLandPhoto(fd);
      if ('error' in result) {
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    const result = await deleteRiversLandPhoto(id);
    if (result.success) {
  ...
  async function onDragEnd() {
    ...
    await reorderRiversLandPhotos(photos.map((p) => ({ id: p.id, order: p.order })));
  }
    ...
    <div className="space-y-6">
      ...
      {photos.length === 0 ? (
        <div className="text-center py-12 text-gray-400 text-sm">
          Nicio fotografie adăugată. Galeria River&apos;s Land va afișa un placeholder până atunci.
        </div>
      ) : (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Fotografia va fi eliminată definitiv din galeria River&apos;s Land.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
